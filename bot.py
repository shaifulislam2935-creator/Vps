...
def create_embed(title, description="", color=0x1a1a1a):
    ...
    embed = discord.Embed(
        title=truncate_text(f"⭐ InfarnoNodes - {title}", 256),
        description=truncate_text(description, 4096),
        color=color
    )

    embed.set_thumbnail(url="https://imgur.com/a/s85r2wU")
    embed.set_footer(text=f"InfarnoNodes VPS Manager • {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                    icon_url="https://imgur.com/a/s85r2wU")
...
def is_admin():
    async def predicate(ctx):
        ...
        raise commands.CheckFailure(f"You need admin permissions to use this command. Contact InfarnoNodes support.")
...
async def get_or_create_vps_role(guild):
    ...
    role = discord.utils.get(guild.roles, name="InfarnoNodes VPS User")
    if role:
    ...
    try:
        role = await guild.create_role(
            name="InfarnoNodes VPS User",
            color=discord.Color.dark_purple(),
            reason="InfarnoNodes VPS User role for bot management",
            permissions=discord.Permissions.none()
        )
        VPS_USER_ROLE_ID = role.id
        logger.info(f"Created InfarnoNodes VPS User role: {role.name} (ID: {role.id})")
        return role
    except Exception as e:
        logger.error(f"Failed to create InfarnoNodes VPS User role: {e}")
        return None
...
async def on_ready():
    ...
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="InfarnoNodes VPS Manager"))
    bot.loop.create_task(vps_monitor())
    logger.info("InfarnoNodes Bot is ready! VPS monitoring started.")
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandNotFound):
        ...
        error_msg = str(error) if str(error) != "Admin required" else "You need admin permissions for this command. Contact InfarnoNodes support."
        ...
        await ctx.send(embed=create_error_embed("System Error", "An unexpected error occurred. InfarnoNodes support has been notified."))
...
async def ping(ctx):
    ...
    embed = create_success_embed("Pong!", f"InfarnoNodes Bot latency: {latency}ms")
...
async def my_vps(ctx):
    ...
    if not vps_list:
        await ctx.send(embed=create_embed("No VPS Found", "You don't have any InfarnoNodes VPS. Contact an admin to create one.", 0xff3366))
        return
    embed = create_info_embed("My InfarnoNodes VPS", "")
    text = []
...
async def lxc_list(ctx):
    ...
    try:
        ...
        embed = create_info_embed("InfarnoNodes LXC Containers List", result)
        await ctx.send(embed=embed)
    except Exception as e:
...
async def create_vps(ctx, ram: int, cpu: int, disk: int, user: discord.Member):
    ...
    container_name = f"infarnonodes-vps-{user_id}-{vps_count}"
    ram_mb = ram * 1024

    await ctx.send(embed=create_info_embed("Creating InfarnoNodes VPS", f"Deploying VPS for {user.mention}..."))
    ...
    try:
        ...
        if ctx.guild:
            ...
            if vps_role:
                try:
                    await user.add_roles(vps_role, reason="InfarnoNodes VPS ownership granted")
                except discord.Forbidden:
                    logger.warning(f"Failed to assign InfarnoNodes VPS role to {user.name}")

        # Create success embed for channel
        embed = create_success_embed("InfarnoNodes VPS Created Successfully")
        ...
        try:
            dm_embed = create_success_embed("InfarnoNodes VPS Created!", f"Your VPS has been successfully deployed by an admin!")
            add_field(dm_embed, "VPS Details", f"**VPS ID:** #{vps_count}\n**Container Name:** `{container_name}`\n**Configuration:** {config_str}\n**Status:** Running\n**Created:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", False)
            add_field(dm_embed, "Management", "• Use `!manage` to start/stop/reinstall your InfarnoNodes VPS\n• Use `!manage` → SSH for terminal access\n• Contact InfarnoNodes admin for upgrades or issues", False)
            add_field(dm_embed, "Important Notes", "• Full root access via SSH\n• Ubuntu 22.04 pre-installed\n• Back up your data regularly with InfarnoNodes tools", False)
...
class ManageView(discord.ui.View):
        ...
        if len(vps_list) > 1:
            options = [
                discord.SelectOption(
                    label=f"InfarnoNodes VPS {i+1} ({v.get('config', 'Custom')})",
                    description=f"Status: {v.get('status', 'unknown')}",
                    value=str(i)
                ) for i, v in enumerate(vps_list)
            ]
            self.select = discord.ui.Select(placeholder="Select a InfarnoNodes VPS to manage", options=options)
            self.select.callback = self.select_vps
            self.add_item(self.select)
            self.initial_embed = create_embed("InfarnoNodes VPS Management", "Select a VPS from the dropdown menu below.", 0x1a1a1a)
    ...
    async def create_vps_embed(self, index):
        ...
        embed = create_embed(
            f"InfarnoNodes VPS Management - VPS {index + 1}",
        ...
        if suspended:
            add_field(embed, "⚠️ Suspended", "This InfarnoNodes VPS is suspended. Contact an admin to unsuspend.", False)

        live_stats = f"**CPU Usage:** {cpu_usage}\n**Memory:** {memory_usage}\n**Disk:** {disk_usage}"
        add_field(embed, "📈 Live Usage", live_stats, False)

        add_field(embed, "🎮 Controls", "Use the buttons below to manage your InfarnoNodes VPS", False)
    ...
    async def select_vps(self, interaction: discord.Interaction):
        if str(interaction.user.id) != self.user_id and not self.is_admin:
            await interaction.response.send_message(embed=create_error_embed("Access Denied", "This is not your InfarnoNodes VPS!"), ephemeral=True)
            return
    ...
    async def action_callback(self, interaction: discord.Interaction, action: str):
        ...
        if suspended and not self.is_admin and action != 'stats':
            await interaction.response.send_message(embed=create_error_embed("Access Denied", "This InfarnoNodes VPS is suspended. Contact an admin to unsuspend."), ephemeral=True)
            return
        ...
        if action == 'stats':
            ...
            memory_usage = await get_container_memory(container_name)
            disk_usage = await get_container_disk(container_name)
            stats_embed = create_info_embed("📈 InfarnoNodes Live Statistics", f"Real-time stats for `{container_name}`")
        ...
        if action == 'reinstall':
            if self.is_shared or self.is_admin:
                await interaction.response.send_message(embed=create_error_embed("Access Denied", "Only the InfarnoNodes VPS owner can reinstall!"), ephemeral=True)
                return
            if suspended:
                await interaction.response.send_message(embed=create_error_embed("Cannot Reinstall", "Unsuspend the InfarnoNodes VPS first."), ephemeral=True)
                return

            confirm_embed = create_warning_embed("InfarnoNodes Reinstall Warning",
            ...
            class ConfirmView(discord.ui.View):
                ...
                async def confirm(self, interaction: discord.Interaction, item: discord.ui.Button):
                    ...
                    try:
                        ...
                        await interaction.followup.send(embed=create_info_embed("Recreating Container", f"Creating new InfarnoNodes container `{self.container_name}`..."), ephemeral=True)
                        ...
                        save_data()
                        await interaction.followup.send(embed=create_success_embed("Reinstall Complete", f"InfarnoNodes VPS `{self.container_name}` has been successfully reinstalled!"), ephemeral=True)
            ...
            try:
                ...
                vps["status"] = "running"
                save_data()
                await interaction.followup.send(embed=create_success_embed("VPS Started", f"InfarnoNodes VPS `{container_name}` is now running!"), ephemeral=True)
                new_embed = await self.create_vps_embed(self.selected_index)
            ...
            try:
                ...
                vps["status"] = "stopped"
                save_data()
                await interaction.followup.send(embed=create_success_embed("VPS Stopped", f"InfarnoNodes VPS `{container_name}` has been stopped!"), ephemeral=True)
                new_embed = await self.create_vps_embed(self.selected_index)
                await interaction.message.edit(embed=new_embed, view=self)
            except Exception as e:
            ...
            if suspended:
                await interaction.response.send_message(embed=create_error_embed("Access Denied", "Cannot access suspended InfarnoNodes VPS."), ephemeral=True)
                return
            await interaction.response.send_message(embed=create_info_embed("SSH Access", "Generating InfarnoNodes SSH connection..."), ephemeral=True)
            ...
            try:
                ...
                if check_proc.returncode != 0:
                    ...
                    await interaction.followup.send(embed=create_success_embed("Installed", "InfarnoNodes SSH service installed!"), ephemeral=True)

                # Start tmate with unique session name using timestamp
                session_name = f"infarnonodes-session-{datetime.now().strftime('%Y%m%d%H%M%S')}"
                ...
                if ssh_url:
                    try:
                        ssh_embed = create_embed("🔑 InfarnoNodes SSH Access", f"SSH connection for VPS `{container_name}`:", 0x00ff88)
                        ...
                        await interaction.followup.send(embed=create_success_embed("SSH Sent", f"Check your DMs for InfarnoNodes SSH link! Session: {session_name}"), ephemeral=True)
                    except discord.Forbidden:
                        await interaction.followup.send(embed=create_error_embed("DM Failed", "Enable DMs to receive InfarnoNodes SSH link!"), ephemeral=True)
                else:
...
async def manage_vps(ctx, user: discord.Member = None):
    """Manage your InfarnoNodes VPS or another user's VPS (Admin only)"""
    ...
    if user:
        ...
        if user_id_check != str(MAIN_ADMIN_ID) and user_id_check not in admin_data.get("admins", []):
            await ctx.send(embed=create_error_embed("Access Denied", "Only InfarnoNodes admins can manage other users' VPS."))
            return
        
        user_id = str(user.id)
        vps_list = vps_data.get(user_id, [])
        if not vps_list:
            await ctx.send(embed=create_error_embed("No VPS Found", f"{user.mention} doesn't have any InfarnoNodes VPS."))
            return
        
        # Create admin view for managing another user's VPS
        view = ManageView(str(ctx.author.id), vps_list, is_admin=True, owner_id=user_id)
        await ctx.send(embed=create_info_embed(f"Managing {user.name}'s InfarnoNodes VPS", f"Managing VPS for {user.mention}"), view=view)
    else:
        # User managing their own VPS
        user_id = str(ctx.author.id)
        vps_list = vps_data.get(user_id, [])
        if not vps_list:
            embed = create_embed("No VPS Found", "You don't have any InfarnoNodes VPS. Contact an admin to create one.", 0xff3366)
            add_field(embed, "Quick Actions", "• `!manage` - Manage VPS\n• Contact InfarnoNodes admin for VPS creation", False)
            await ctx.send(embed=embed)
            return
...
async def list_all_vps(ctx):
    """List all InfarnoNodes VPS and user information (Admin only)"""
    total_vps = 0
    ...
    for user_id, vps_list in vps_data.items():
        try:
            ...
            # User summary
            user_summary.append(f"**{user.name}** ({user.mention}) - {user_vps_count} InfarnoNodes VPS ({user_running} running, {user_suspended} suspended)")
            ...
            vps_info.append(f"❓ Unknown User ({user_id}) - {len(vps_list)} InfarnoNodes VPS")
    
    # Create multiple embeds if needed to avoid character limit
    embeds = []
    
    # First embed with overview
    embed = create_embed("All InfarnoNodes VPS Information", "Complete overview of all InfarnoNodes VPS deployments and user statistics", 0x1a1a1a)
    add_field(embed, "System Overview", f"**Total Users:** {total_users}\n**Total VPS:** {total_vps}\n**Running:** {running_vps}\n**Stopped:** {stopped_vps}\n**Suspended:** {suspended_vps}", False)
    embeds.append(embed)
    
    # User summary embed
    if user_summary:
        embed = create_embed("InfarnoNodes User Summary", f"Summary of all users and their InfarnoNodes VPS", 0x1a1a1a)
    ...
    if vps_info:
        ...
        for i in range(0, len(vps_info), 15):
            ...
            embed = create_embed(f"InfarnoNodes VPS Details ({i+1}-{min(i+15, len(vps_info))})", "List of all InfarnoNodes VPS deployments", 0x1a1a1a)
...
async def manage_shared_vps(ctx, owner: discord.Member, vps_number: int):
    """Manage a shared InfarnoNodes VPS"""
    owner_id = str(owner.id)
    user_id = str(ctx.author.id)
    if owner_id not in vps_data or vps_number < 1 or vps_number > len(vps_data[owner_id]):
        await ctx.send(embed=create_error_embed("Invalid VPS", "Invalid VPS number or owner doesn't have a InfarnoNodes VPS."))
        return
    vps = vps_data[owner_id][vps_number - 1]
    if user_id not in vps.get("shared_with", []):
        await ctx.send(embed=create_error_embed("Access Denied", "You do not have access to this InfarnoNodes VPS."))
        return
...
async def share_user(ctx, shared_user: discord.Member, vps_number: int):
    """Share InfarnoNodes VPS access with another user"""
    user_id = str(ctx.author.id)
    shared_user_id = str(shared_user.id)
    if user_id not in vps_data or vps_number < 1 or vps_number > len(vps_data[user_id]):
        await ctx.send(embed=create_error_embed("Invalid VPS", "Invalid VPS number or you don't have a InfarnoNodes VPS."))
        return
    ...
    if shared_user_id in vps["shared_with"]:
        await ctx.send(embed=create_error_embed("Already Shared", f"{shared_user.mention} already has access to this Infarno
Nodes VPS!"))
        return
    vps["shared_with"].append(shared_user_id)
    save_data()
    await ctx.send(embed=create_success_embed("VPS Shared", f"InfarnoNodes VPS #{vps_number} shared with {shared_user.mention}!"))
    try:
        await shared_user.send(embed=create_embed("InfarnoNodes VPS Access Granted", f"You have access to VPS #{vps_number} from {ctx.author.mention}. Use `!manage-shared {ctx.author.mention} {vps_number}`", 0x00ff88))
    except discord.Forbidden:
...
async def revoke_share(ctx, shared_user: discord.Member, vps_number: int):
    """Revoke shared InfarnoNodes VPS access"""
    user_id = str(ctx.author.id)
    shared_user_id = str(shared_user.id)
    if user_id not in vps_data or vps_number < 1 or vps_number > len(vps_data[user_id]):
        await ctx.send(embed=create_error_embed("Invalid VPS", "Invalid VPS number or you don't have a InfarnoNodes VPS."))
    ...
    if shared_user_id not in vps["shared_with"]:
        await ctx.send(embed=create_error_embed("Not Shared", f"{shared_user.mention} doesn't have access to this InfarnoNodes VPS!"))
        return
    vps["shared_with"].remove(shared_user_id)
    save_data()
    await ctx.send(embed=create_success_embed("Access Revoked", f"Access to InfarnoNodes VPS #{vps_number} revoked from {shared_user.mention}!"))
    try:
        await shared_user.send(embed=create_embed("InfarnoNodes VPS Access Revoked", f"Your access to VPS #{vps_number} by {ctx.author.mention} has been revoked.", 0xff3366))
    except discord.Forbidden:
        await ctx.send(embed=create_info_embed("Notification Failed", f"Could not DM {shared_user.mention}"))
...
async def delete_vps(ctx, user: discord.Member, vps_number: int, *, reason: str = "No reason"):
    """Delete a user's InfarnoNodes VPS (Admin only)"""
    user_id = str(user.id)
    if user_id not in vps_data or vps_number < 1 or vps_number > len(vps_data[user_id]):
        await ctx.send(embed=create_error_embed("Invalid VPS", "Invalid VPS number or user doesn't have a InfarnoNodes VPS."))
        return
    vps = vps_data[user_id][vps_number - 1]
    container_name = vps["container_name"]

    await ctx.send(embed=create_info_embed("Deleting InfarnoNodes VPS", f"Removing VPS #{vps_number}..."))
        ...
        if not vps_data[user_id]:
            ...
            if ctx.guild:
                ...
                if vps_role and vps_role in user.roles:
                    try:
                        await user.remove_roles(vps_role, reason="No InfarnoNodes VPS ownership")
                    except discord.Forbidden:
                        logger.warning(f"Failed to remove InfarnoNodes VPS role from {user.name}")
        save_data()

        embed = create_success_embed("InfarnoNodes VPS Deleted Successfully")
        add_field(embed, "Owner", user.mention, True)
...
async def add_resources(ctx, vps_id: str, ram: int = None, cpu: int = None, disk: int = None):
    """Add resources to a InfarnoNodes VPS (Admin only)"""
    ...
    if not found_vps:
        await ctx.send(embed=create_error_embed("VPS Not Found", f"No InfarnoNodes VPS found with ID: `{vps_id}`"))
        return
    
    was_running = found_vps.get('status') == 'running' and not found_vps.get('suspended', False)
    if was_running:
        await ctx.send(embed=create_info_embed("Stopping VPS", f"Stopping InfarnoNodes VPS `{vps_id}` to apply resource changes..."))
        try:
    ...
    try:
        ...
        embed = create_success_embed("Resources Added", f"Successfully added resources to InfarnoNodes VPS `{vps_id}`")
...
async def admin_add(ctx, user: discord.Member):
    """Add InfarnoNodes admin (Main admin only)"""
    user_id = str(user.id)
    if user_id == str(MAIN_ADMIN_ID):
        await ctx.send(embed=create_error_embed("Already Admin", "This user is already the main InfarnoNodes admin!"))
        return

    if user_id in admin_data.get("admins", []):
        await ctx.send(embed=create_error_embed("Already Admin", f"{user.mention} is already a InfarnoNodes admin!"))
        return
    ...
    save_data()
    await ctx.send(embed=create_success_embed("Admin Added", f"{user.mention} is now a InfarnoNodes admin!"))
    try:
        await user.send(embed=create_embed("🎉 InfarnoNodes Admin Role Granted", f"You are now a InfarnoNodes admin by {ctx.author.mention}", 0x00ff88))
    except discord.Forbidden:
...
async def admin_remove(ctx, user: discord.Member):
    """Remove InfarnoNodes admin (Main admin only)"""
    user_id = str(user.id)
    if user_id == str(MAIN_ADMIN_ID):
        await ctx.send(embed=create_error_embed("Cannot Remove", "You cannot remove the main InfarnoNodes admin!"))
        return

    if user_id not in admin_data.get("admins", []):
        await ctx.send(embed=create_error_embed("Not Admin", f"{user.mention} is not a InfarnoNodes admin!"))
        return

    admin_data["admins"].remove(user_id)
    save_data()
    await ctx.send(embed=create_success_embed("Admin Removed", f"{user.mention} is no longer a InfarnoNodes admin!"))
    try:
        await user.send(embed=create_embed("⚠️ Infarno
Nodes Admin Role Revoked", f"Your admin role was removed by {ctx.author.mention}", 0xff3366))
    except discord.Forbidden:
        await ctx.send(embed=create_info_embed("Notification Failed", f"Could not DM {user.mention}"))
...
async def admin_list(ctx):
    """List all InfarnoNodes admins (Main admin only)"""
    admins = admin_data.get("admins", [])
    main_admin = await bot.fetch_user(MAIN_ADMIN_ID)

    embed = create_embed("👑 InfarnoNodes Admin Team", "Current InfarnoNodes administrators:", 0x1a1a1a)
    ...
    if admins:
        ...
        add_field(embed, "🛡️ Admins", "No additional InfarnoNodes admins", False)
...
async def user_info(ctx, user: discord.Member):
    """Get detailed information about a InfarnoNodes user (Admin only)"""
    user_id = str(user.id)

    # Get user's VPS
    vps_list = vps_data.get(user_id, [])

    embed = create_embed(f"Infarno
Nodes User Information - {user.name}", f"Detailed information for {user.mention}", 0x1a1a1a)
    ...
    if vps_list:
        ...
        add_field(embed, "🖥️ InfarnoNodes VPS Information", vps_summary, False)
        ...
        if len(vps_info) > 10:
            ...
            for i in range(10, len(vps_info), 10):
                ...
                additional_embed = create_embed(f"InfarnoNodes VPS List ({i+1}-{min(i+10, len(vps_info))})", f"More VPS for {user.mention}", 0x1a1a1a)
        ...
        add_field(embed, "🖥️ InfarnoNodes VPS Information", "**No VPS owned**", False)
        await ctx.send(embed=embed)

    # Check if user is admin
    is_admin_user = user_id == str(MAIN_ADMIN_ID) or user_id in admin_data.get("admins", [])
    add_field(embed, "🛡️ InfarnoNodes Admin Status", f"**{'Yes' if is_admin_user else 'No'}**", False)

@bot.command(name='serverstats')
@is_admin()
async def server_stats(ctx):
    """Show InfarnoNodes server statistics (Admin only)"""
    total_users = len(vps_data)
    ...
    embed = create_embed("📊 InfarnoNodes Server Statistics", "Current InfarnoNodes server overview", 0x1a1a1a)
...
async def vps_info(ctx, container_name: str = None):
    """Get detailed InfarnoNodes VPS information (Admin only)"""
    if not container_name:
        ...
        for user_id, vps_list in vps_data.items():
            try:
                ...
                for i, vps in enumerate(vps_list):
                    ...
                    if vps.get('suspended', False):
                        status_text += " (SUSPENDED)"
                    all_vps.append(f"**{user.name}** - InfarnoNodes VPS {i+1}: `{vps['container_name']}` - {status_text}")
            except:
                pass
        ...
        for i in range(0, len(all_vps), 20):
            ...
            embed = create_embed(f"🖥️ All InfarnoNodes VPS ({i+1}-{min(i+20, len(all_vps))})", f"List of all InfarnoNodes VPS deployments", 0x1a1a1a)
            add_field(embed, "VPS List", "\n".join(chunk), False)
            await ctx.send(embed=embed)
    else:
        ...
        if not found_vps:
            await ctx.send(embed=create_error_embed("VPS Not Found", f"No InfarnoNodes VPS found with container name: `{container_name}`"))
            return

        suspended_text = " (SUSPENDED)" if found_vps.get('suspended', False) else ""
        embed = create_embed(f"🖥️ InfarnoNodes VPS Information - {container_name}", f"Details for VPS owned by {found_user.mention}{suspended_text}", 0x1a1a1a)
...
async def restart_vps(ctx, container_name: str):
    """Restart a InfarnoNodes VPS (Admin only)"""
    await ctx.send(embed=create_info_embed("Restarting VPS", f"Restarting InfarnoNodes VPS `{container_name}`..."))
    ...
    try:
        ...
        await ctx.send(embed=create_success_embed("VPS Restarted", f"InfarnoNodes VPS `{container_name}` has been restarted successfully!"))
...
async def backup_vps(ctx, container_name: str):
    """Create a snapshot of a InfarnoNodes VPS (Admin only)"""
    snapshot_name = f"infarnonodes-{container_name}-backup-{datetime.now().strftime('%Y%m%d-%H%M%S')}"

    await ctx.send(embed=create_info_embed("Creating InfarnoNodes Backup", f"Creating snapshot of `{container_name}`..."))

    try:
        await execute_lxc(f"lxc snapshot {container_name} {snapshot_name}")
        await ctx.send(embed=create_success_embed("Backup Created", f"InfarnoNodes Snapshot `{snapshot_name}` created successfully!"))
...
async def restore_vps(ctx, container_name: str, snapshot_name: str):
    """Restore a InfarnoNodes VPS from snapshot (Admin only)"""
    await ctx.send(embed=create_info_embed("Restoring VPS", f"Restoring `{container_name}` from InfarnoNodes snapshot `{snapshot_name}`..."))

    try:
        await execute_lxc(f"lxc restore {container_name} {snapshot_name}")
        await ctx.send(embed=create_success_embed("VPS Restored", f"InfarnoNodes VPS `{container_name}` has been restored from snapshot!"))
...
async def list_snapshots(ctx, container_name: str):
    """List all snapshots for a InfarnoNodes VPS (Admin only)"""
    try:
        ...
        if snapshots:
            ...
            for i in range(0, len(snapshots), 20):
                ...
                embed = create_embed(f"📸 InfarnoNodes Snapshots for {container_name} ({i+1}-{min(i+20, len(snapshots))})", f"List of snapshots", 0x1a1a1a)
                add_field(embed, "Snapshots", "\n".join([f"• {snap}" for snap in chunk]), False)
                await ctx.send(embed=embed)
        else:
            await ctx.send(embed=create_info_embed("No Snapshots", f"No InfarnoNodes snapshots found for `{container_name}`"))
...
async def execute_command(ctx, container_name: str, *, command: str):
    """Execute a command inside a InfarnoNodes VPS (Admin only)"""
    await ctx.send(embed=create_info_embed("Executing Command", f"Running command in InfarnoNodes VPS `{container_name}`..."))
...
async def stop_all_vps(ctx):
    """Stop all InfarnoNodes VPS using lxc stop --all --force (Admin only)"""
    await ctx.send(embed=create_warning_embed("Stopping All InfarnoNodes VPS", "⚠️ **WARNING:** This will stop ALL running VPS on the InfarnoNodes server.\n\nThis action cannot be undone. Continue?"))
    ...
    class ConfirmView(discord.ui.View):
        ...
        async def confirm(self, interaction: discord.Interaction, item: discord.ui.Button):
            ...
            try:
                ...
                if proc.returncode == 0:
                    ...
                    embed = create_success_embed("All InfarnoNodes VPS Stopped", f"Successfully stopped {stopped_count} VPS using `lxc stop --all --force`")
                    output_text = stdout.decode() if stdout else 'No output'
                    add_field(embed, "Command Output", f"```\n{output_text}\n```", False)
                    await interaction.followup.send(embed=embed)
                else:
                    error_msg = stderr.decode() if stderr else "Unknown error"
                    embed = create_error_embed("Stop Failed", f"Failed to stop InfarnoNodes VPS: {error_msg}")
                    await interaction.followup.send(embed=embed)
        ...
        @discord.ui.button(label="Cancel", style=discord.ButtonStyle.secondary)
        async def cancel(self, interaction: discord.Interaction, item: discord.ui.Button):
            await interaction.response.edit_message(embed=create_info_embed("Operation Cancelled", "The stop all InfarnoNodes VPS operation has been cancelled."))
...
async def cpu_monitor_control(ctx, action: str = "status"):
    """Control InfarnoNodes CPU monitoring system (Admin only)"""
    global cpu_monitor_active
    
    if action.lower() == "status":
        status = "Active" if cpu_monitor_active else "Inactive"
        embed = create_embed("InfarnoNodes CPU Monitor Status", f"InfarnoNodes CPU monitoring is currently **{status}**", 0x00ccff if cpu_monitor_active else 0xffaa00)
        add_field(embed, "Threshold", f"{CPU_THRESHOLD}% CPU usage", True)
        add_field(embed, "Check Interval", f"60 seconds (host)", True)
        await ctx.send(embed=embed)
    elif action.lower() == "enable":
        cpu_monitor_active = True
        await ctx.send(embed=create_success_embed("CPU Monitor Enabled", "InfarnoNodes CPU monitoring has been enabled."))
    elif action.lower() == "disable":
        cpu_monitor_active = False
        await ctx.send(embed=create_warning_embed("CPU Monitor Disabled", "InfarnoNodes CPU monitoring has been disabled."))
    else:
...
async def resize_vps(ctx, container_name: str, ram: int = None, cpu: int = None, disk: int = None):
    """Resize InfarnoNodes VPS resources (Admin only)"""
    if ram is None and cpu is None and disk is None:
    ...
    was_running = found_vps.get('status') == 'running' and not found_vps.get('suspended', False)
    if was_running:
        await ctx.send(embed=create_info_embed("Stopping VPS", f"Stopping InfarnoNodes VPS `{container_name}` to apply resource changes..."))
        try:
    ...
    try:
        ...
        embed = create_success_embed("VPS Resized", f"Successfully resized resources for InfarnoNodes VPS `{container_name}`")
        add_field(embed, "Changes Applied", "\n".join(changes), False)
        await ctx.send(embed=embed)
...
async def clone_vps(ctx, container_name: str, new_name: str = None):
    """Clone a InfarnoNodes VPS (Admin only)"""
    if not new_name:
        # Generate a new name if not provided
        timestamp = datetime.now().strftime('%Y%m%d-%H%M%S')
        new_name = f"infarnonodes-{container_name}-clone-{timestamp}"
    
    await ctx.send(embed=create_info_embed("Cloning VPS", f"Cloning InfarnoNodes VPS `{container_name}` to `{new_name}`..."))
    ...
    try:
        ...
        embed = create_success_embed("VPS Cloned", f"Successfully cloned InfarnoNodes VPS `{container_name}` to `{new_name}`")
...
async def migrate_vps(ctx, container_name: str, target_pool: str):
    """Migrate a InfarnoNodes VPS to a different storage pool (Admin only)"""
    await ctx.send(embed=create_info_embed("Migrating VPS", f"Migrating InfarnoNodes VPS `{container_name}` to storage pool `{target_pool}`..."))
    ...
    try:
        ...
        temp_name = f"infarnonodes-{container_name}-temp-{int(time.time())}"
        ...
        await ctx.send(embed=create_success_embed("VPS Migrated", f"Successfully migrated InfarnoNodes VPS `{container_name}` to storage pool `{target_pool}`"))
...
async def vps_stats(ctx, container_name: str):
    """Show detailed resource usage statistics for a InfarnoNodes VPS (Admin only)"""
    await ctx.send(embed=create_info_embed("Gathering Statistics", f"Collecting statistics for InfarnoNodes VPS `{container_name}`..."))
    ...
    try:
        ...
        embed = create_embed(f"📊 InfarnoNodes VPS Statistics - {container_name}", f"Resource usage statistics", 0x1a1a1a)
...
async def vps_network(ctx, container_name: str, action: str, value: str = None):
    """Manage InfarnoNodes VPS network settings (Admin only)"""
    ...
    try:
        if action.lower() == "list":
            ...
            if proc.returncode == 0:
                ...
                embed = create_embed(f"🌐 InfarnoNodes Network Interfaces - {container_name}", "Network configuration", 0x1a1a1a)
            ...
            await ctx.send(embed=create_success_embed("Network Limited", f"Set InfarnoNodes network limit to {value} for `{container_name}`"))
        
        elif action.lower() in ["add", "remove"]:
            await ctx.send(embed=create_info_embed("Not Implemented", f"InfarnoNodes Network {action} is not yet implemented. Use list or limit for now."))
...
async def vps_processes(ctx, container_name: str):
    """Show running processes in a InfarnoNodes VPS (Admin only)"""
    await ctx.send(embed=create_info_embed("Gathering Processes", f"Listing processes in InfarnoNodes VPS `{container_name}`..."))
    ...
    try:
        ...
        if proc.returncode == 0:
            ...
            embed = create_embed(f"⚙️ InfarnoNodes Processes - {container_name}", "Running processes", 0x1a1a1a)
...
async def vps_logs(ctx, container_name: str, lines: int = 50):
    """Show recent logs from a InfarnoNodes VPS (Admin only)"""
    await ctx.send(embed=create_info_embed("Gathering Logs", f"Fetching last {lines} lines from InfarnoNodes VPS `{container_name}`..."))
    ...
    try:
        ...
        if proc.returncode == 0:
            ...
            embed = create_embed(f"📋 InfarnoNodes Logs - {container_name}", f"Last {lines} log lines", 0x1a1a1a)
...
async def suspend_vps(ctx, container_name: str, *, reason: str = "Admin action"):
    """Suspend a InfarnoNodes VPS (Admin only)"""
    found = False
    for uid, lst in vps_data.items():
        for vps in lst:
            if vps['container_name'] == container_name:
                if vps.get('status') != 'running':
                    await ctx.send(embed=create_error_embed("Cannot Suspend", "InfarnoNodes VPS must be running to suspend."))
                    return
                try:
                    await execute_lxc(f"lxc stop {container_name}")
                    vps['status'] = 'suspended'
                ...
                try:
                    ...
                    embed = create_warning_embed("🚨 InfarnoNodes VPS Suspended", f"Your VPS `{container_name}` has been suspended by an admin.\n\n**Reason:** {reason}\n\nContact a InfarnoNodes admin to unsuspend.")
                    await owner.send(embed=embed)
                except Exception as dm_e:
                    logger.error(f"Failed to DM owner {uid}: {dm_e}")
                await ctx.send(embed=create_success_embed("VPS Suspended", f"InfarnoNodes VPS `{container_name}` suspended. Reason: {reason}"))
                found = True
                break
        if found:
            break
    if not found:
        await ctx.send(embed=create_error_embed("Not Found", f"InfarnoNodes VPS `{container_name}` not found."))

@bot.command(name='unsuspend-vps')
@is_admin()
async def unsuspend_vps(ctx, container_name: str):
    """Unsuspend a InfarnoNodes VPS (Admin only)"""
    found = False
    for uid, lst in vps_data.items():
        for vps in lst:
            if vps['container_name'] == container_name:
                if not vps.get('suspended', False):
                    await ctx.send(embed=create_error_embed("Not Suspended", "InfarnoNodes VPS is not suspended."))
                    return
                try:
                    vps['suspended'] = False
                    vps['status'] = 'running'
                    await execute_lxc(f"lxc start {container_name}")
                    save_data()
                    await ctx.send(embed=create_success_embed("VPS Unsuspended", f"InfarnoNodes VPS `{container_name}` unsuspended and started."))
                    found = True
                except Exception as e:
...
async def suspension_logs(ctx, container_name: str = None):
    """View InfarnoNodes suspension logs (Admin only)"""
    if container_name:
        # Specific VPS
        found = None
        ...
        if not history:
            await ctx.send(embed=create_info_embed("No Suspensions", f"No InfarnoNodes suspension history for `{container_name}`."))
            return
        embed = create_embed("InfarnoNodes Suspension History", f"For `{container_name}`")
        text = []
        ...
        if not all_logs:
            await ctx.send(embed=create_info_embed("No Suspensions", "No InfarnoNodes suspension events recorded."))
            return
        # Split into embeds
        for i in range(0, len(all_logs), 10):
            chunk = all_logs[i:i+10]
            embed = create_embed(f"InfarnoNodes Suspension Logs ({i+1}-{min(i+10, len(all_logs))})", f"Global suspension events (newest first)")
            add_field(embed, "Events", "\n".join(chunk), False)
            await ctx.send(embed=embed)

@bot.command(name='help')
async def show_help(ctx):
    """Show InfarnoNodes help information"""
    user_id = str(ctx.author.id)
    ...
    embed = create_embed("📚 InfarnoNodes Command Help - User Commands", "InfarnoNodes VPS Manager Commands:", 0x1a1a1a)

    user_commands = [
        ("!ping", "Check InfarnoNodes bot latency"),
        ("!uptime", "Show host uptime"),
        ("!myvps", "List your InfarnoNodes VPS"),
        ("!manage [@user]", "Manage your VPS or another user's VPS (Admin only)"),
        ("!share-user @user <vps_number>", "Share InfarnoNodes VPS access"),
        ("!share-ruser @user <vps_number>", "Revoke InfarnoNodes VPS access"),
        ("!manage-shared @owner <vps_number>", "Manage shared InfarnoNodes VPS")
    ]
    ...
    if is_user_admin:
        ...
        embed = create_embed("📚 InfarnoNodes Command Help - Admin Commands", "InfarnoNodes VPS Manager Commands:", 0x1a1a1a)

        admin_commands = [
            ("!lxc-list", "List all LXC containers"),
            ("!create <ram_gb> <cpu_cores> <disk_gb> @user", "Create custom InfarnoNodes VPS"),
            ("!delete-vps @user <vps_number> <reason>", "Delete user's InfarnoNodes VPS"),
            ("!add-resources <vps_id> [ram] [cpu] [disk]", "Add resources to a InfarnoNodes VPS"),
            ("!resize-vps <container> [ram] [cpu] [disk]", "Resize InfarnoNodes VPS resources"),
            ("!suspend-vps <container> [reason]", "Suspend a InfarnoNodes VPS"),
            ("!unsuspend-vps <container>", "Unsuspend a InfarnoNodes VPS"),
            ("!suspension-logs [container]", "View InfarnoNodes suspension logs"),
            ("!userinfo @user", "Get detailed InfarnoNodes user information"),
            ("!serverstats", "Show InfarnoNodes server statistics"),
            ("!vpsinfo [container]", "Get InfarnoNodes VPS information"),
            ("!list-all", "View all InfarnoNodes VPS and user information"),
            ("!restart-vps <container>", "Restart a InfarnoNodes VPS"),
            ("!backup-vps <container>", "Create InfarnoNodes VPS snapshot"),
            ("!restore-vps <container> <snapshot>", "Restore from InfarnoNodes snapshot"),
            ("!list-snapshots <container>", "List Infarno
Nodes VPS snapshots"),
            ("!exec <container> <command>", "Execute command in InfarnoNodes VPS"),
            ("!stop-vps-all", "Stop all InfarnoNodes VPS with lxc stop --all --force"),
            ("!cpu-monitor <status|enable|disable>", "Control InfarnoNodes CPU monitoring system"),
            ("!clone-vps <container> [new_name]", "Clone a InfarnoNodes VPS"),
            ("!migrate-vps <container> <pool>", "Migrate InfarnoNodes VPS to storage pool"),
            ("!vps-stats <container>", "Show InfarnoNodes VPS resource stats"),
            ("!vps-network <container> <action> [value]", "Manage InfarnoNodes network"),
            ("!vps-processes <container>", "List InfarnoNodes processes"),
            ("!vps-logs <container> [lines]", "Show InfarnoNodes system logs")
        ]
    ...
    if is_user_main_admin:
        ...
        embed = create_embed("📚 InfarnoNodes Command Help - Main Admin Commands", "InfarnoNodes VPS Manager Commands:", 0x1a1a1a)

        main_admin_commands = [
            ("!admin-add @user", "Promote to InfarnoNodes admin"),
            ("!admin-remove @user", "Remove InfarnoNodes admin"),
            ("!admin-list", "View all InfarnoNodes admins")
        ]
        ...
        embed.set_footer(text="InfarnoNodes VPS Manager • Auto-suspend on high usage • Enhanced monitoring")
        await ctx.send(embed=embed)
...
async def manage_typo(ctx):
    ...
    await ctx.send(embed=create_info_embed("Command Correction", "Did you mean `!manage`? Use the correct InfarnoNodes command."))
...
async def stats_alias(ctx):
    ...
    if str(ctx.author.id) == str(MAIN_ADMIN_ID) or str(ctx.author.id) in admin_data.get("admins", []):
        ...
        await ctx.send(embed=create_error_embed("Access Denied", "This InfarnoNodes command requires admin privileges."))
